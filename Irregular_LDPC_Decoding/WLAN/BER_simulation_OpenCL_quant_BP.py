...
from Continous_LDPC_Decoding.bp_decoder_irreg import BeliefPropagationDecoderClassIrregular as LDPC_Decoder
...
__doc__ = """This script sets up a proper simulation environment to analyse a belief propagation decoder that receives
          only a limited number of distinct, quantized LLRs. The BER performance of the chosen decoder can be stored
          and compared."""
...
decoder_name = 'quant_bp'+str(cardinality_T_decoder_ops)
...
while not ready:
    ...
    while errors < min_errors:
        ...
        decoded_mat = decodi.decode_OpenCL_belief_propagation(rec_data_quantized,buffer_in=True,return_buffer=True)
        errors += decodi.return_errors_all_zero(decoded_mat)
        transmitted_blocks += + msg_at_time

        # decoded_mat = decodi.decode_on_host(rec_data_quantized.get())
        # errors += np.sum(decoded_mat < 0)
        # transmitted_blocks += + msg_at_time
